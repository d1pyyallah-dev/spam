...
async def test_proxy(proxy):
    try:
        proxy_tuple = ('http', proxy['ip'], proxy['port'], proxy['login'], proxy['password'])
...
async def handle_phone(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("Проверка прокси, подождите...")
    valid_proxies = []
    for p in proxy_list:
        if await test_proxy(p):
            valid_proxies.append(p)
    if not valid_proxies:
        await update.message.reply_text("Нет рабочих прокси, спам невозможен.")
        return
    await update.message.reply_text(f"Найдено {len(valid_proxies)} рабочих прокси. Запуск спама...")
    ...
    async def spam():
        ...
        try:
            while True:
                proxy = valid_proxies[idx % len(valid_proxies)]
                idx += 1
                proxy_tuple = ('http', proxy['ip'], proxy['port'], proxy['login'], proxy['password'])
                try:
                    ...
                    await context.bot.send_message(chat_id, f"FloodWait {e.seconds} сек на прокси {proxy['ip']}, пропускаем.")
                    await asyncio.sleep(1)
                except Exception as e:
                    err_msg = str(e)
                    if "all available options" in err_msg or "ResendCodeRequest" in err_msg:
                        await context.bot.send_message(chat_id, f"Все методы использованы на {proxy['ip']}, меняем прокси.")
                    else:
                        await context.bot.send_message(chat_id, f"Ошибка на {proxy['ip']}: {e}")
                await asyncio.sleep(1)
        except asyncio.CancelledError:
            await client.disconnect()
            raise
